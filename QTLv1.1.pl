...
#	-cov	the average sequencing depth(if using any other software for mapping and SNP calling, make sure this option has been assigned. default:0)
# Output: prefix1.sam prefix1.sorted.bam prefix1.snp.txt prefix2.sam prefix2.sorted.bam prefix2.snp.txt ...
#
# Examples: perl QTLv1.1.pl -f /path_to_ReferSeq/chr.fa -C 4 -FL H_1.fq -FR H_2.fq -SL L_1.fq -SR L_2.fq -D 30 -freq 0.03 -m 0.3 -rn 7 -fp H -sp L
...
#		print $RH "			xmax = max(data[,2]);\n";
		...
		print $RH "			plot(data[,2],data[,3],pch=16,col='darkgreen',main=x,xlab='chromosome position(Mb)',ylab='SNP-index',font.axis=2,font.lab=2,xlim=c(0,$xmax),ylim=c(0,1),type='p',las=1,cex=0.5);\n";
...
#		print $SH "			xmin = min(data[,1]); xmax = max(data[,1]);\n";
		...
		print $SH "			plot(data[,2],data[,3],pch=16,col='darkgoldenrod1',main=x,xlab='chromosome position(Mb)',ylab='SNP-index',font.axis=2,font.lab=2,xlim=c(0,$xmax),ylim=c(0,1),type='p',las=1,cex=0.5);\n";
...
#		print $CH "			xmin = min(data[,1]); xmax = max(data[,1]);\n";
		...
		print $CH "			plot(data[,2],data[,3],pch=16,col='darkblue',main=x,xlab='chromosome position(Mb)',ylab=expression(Delta(SNP-index)),font.axis=2,font.lab=2,xlim=c(0,$xmax),ylim=c(-1,1),type='p',las=1,cex=0.5);\n";
